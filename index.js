...
function processSum(numberList, callback) {
  const totalSum = numberList.reduce(function(acc, numberList){
...
function firstNamesAllCaps(runners) {
  const upperCase = runners.map((item)=> item.first_name.toUpperCase());
  return upperCase;
}
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  const runnerShirts = runners.filter(function(item){
...
function tallyUpDonations(runners) {
  const donationTally = runners.reduce(function(acc, curr){
    // console.log(`I am the accumulator ${acc}`);
    // console.log(`I am the current value ${curr.donation}`);
...
function counterMakerWithLimit(max) {
  let counting = -1;
  return function counterMakerLimit(){
    if(counting === max){
      counting = 0;
    }
    else {
